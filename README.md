...
### 핵심 기능
- **OSM 데이터 파싱**: OpenStreetMap에서 지정된 지역 도로 데이터 추출
...
|     route_viewer_kakao.html 파일에 자바스크립트  키 입력            │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 3️  Supabase 테이블 생성                                           │
...
│     이미 테이블을 만든 DB를 사용할 예정(테스트 필요 시 카톡에 키 확인)   |
│    ( 1분 소요 )                                                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 4️  경사도 데이터 전처리 (시간 소요!) //이 전에 서버 실행해야됨          │
│     python preprocess_edges.py                                  │
│     (몇 시간 소요)                                                │
└─────────────────────────────────────────────────────────────────┘

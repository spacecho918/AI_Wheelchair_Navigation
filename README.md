...
│ 모드        │ 설명                                     │
├─────────────┼──────────────────────────────────────────┤
│ shortest    │ 최단 거리 우선, 경사도 12°까지 허용      │
│ safest      │ 안전 우선, 경사도 5° 초과 시 높은 패널티 │
...
DEM_SAMPLE_INTERVAL=5        # 샘플링 간격 (m)
```
...
### Q: 서버 시작이 느려요
A: 최초 시작 시 OSM 데이터를 다운로드합니다. 두 번째 실행부터는 캐시되어 빠릅니다.

### Q: 경사도가 0으로 나와요
A: `preprocess_edges.py`를 먼저 실행했는지 확인하세요. 실행 후 `edges_data.json` 파일이 생성되어야 합니다.

### Q: Supabase 연결이 안 돼요
A: `.env` 파일에 올바른 SUPABASE_URL과 SUPABASE_KEY가 설정되어 있는지 확인하세요.

### Q: 지역을 변경하고 싶어요
A: `osm_parser.py`의 `BBOX_NORTH/SOUTH/EAST/WEST` 값을 수정 후 `preprocess_edges.py`를 다시 실행하세요.
